...
// motor groups
MotorGroup leftMotors({1,-3,5},MotorGearset::blue); // left motor group - ports 3 (reversed), 4, 5 (reversed)
MotorGroup rightMotors({6,-8,10}, MotorGearset::blue); // right motor group - ports 6, 7, 9 (reversed)

//parameter variables
int inertial_sensor_port(12);
int horizontal_tracking_wheel_port(19);
int vertical_tracking_wheel_port(-11);
float horizontal_tracking_wheel_offset(-5.75); // in inches, negative if the wheel is to the back of the tracking center, positive if it's to the front
float vertical_tracking_wheel_offset(-2.5); // in inches, negative if the wheel
int track_width(10); // in inches
int drivetrain_rpm(360);
...
TrackingWheel horizontal(&horizontalEnc, Omniwheel::NEW_275, horizontal_tracking_wheel_offset);
// vertical tracking wheel. 2.75" diameter, 2.5" offset, left of the robot (negative)
TrackingWheel vertical(&verticalEnc, Omniwheel::NEW_275, vertical_tracking_wheel_offset);
